...
export function clearSecurityMessCache(userId) {
    if (userId) {
...
const activeResidentCache = new Map();
    ...
    try {
        ...
        for (const f of paidMessFees) {
            if (!mealPlanMap.has(f.studentId)) {
                mealPlanMap.set(f.studentId, f.mealPlan || 'VEG');
            }
        }
 ...
 */
async function getActiveResidentInfo(studentProfileId) {
    const now = Date.now();
    ...
    try {
        ...
        const info = {
            ...
            roomNumber,
            hostelName,
            mealPlan: paidFee?.mealPlan || null,
            expiresAt: now + RESIDENT_SYNC_INTERVAL_MS,
        };
...
export class MessEntryService {
     ...
     */
    async getSecurityMess(securityUserId) {
     ...
     */
    async verifyMessEntry(securityUserId, qrToken) {
        ...
        console.log(`[MESS SCAN ULTRA-FAST] total: ${(tEnd - t0).toFixed(2)}ms | student: ${studentUsn} | todayCount: ${updatedCount}`);
     ...
     */
    async getTodayStats(securityUserId) {
        const { messId, messName } = await this.getSecurityMess(securityUserId);
        const todayStr = getTodayDateStr();
...
//# sourceMappingURL=mess-entry.service.js.map