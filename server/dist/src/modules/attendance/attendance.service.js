...
export class AttendanceService {
    ...
    async getActiveSession(securityUserId) {
        const hostelId = await this.getSecurityHostelId(securityUserId);
        const date = this.todayDate();
    ...
    async endSession(securityUserId) {
        ...
        if (!session)
            throw ApiError.notFound("No active session found for today.");
        if (session.status === "COMPLETED")
            throw ApiError.conflict("Session already completed.");
        if (session.securityId !== securityUserId)
            throw ApiError.forbidden("You did not start this session.");
    ...
    async getRegister(hostelId, date) {
    ...
    async exportRegisterCSV(hostelId, dateStr) {
        ...
        const rows = data.register.map((r) => `${r.usn},"${r.studentName}",${r.roomNumber},${r.status},${r.scannedAt ? new Date(r.scannedAt).toLocaleString() : ""}`);
        return [header, ...rows].join("\n");
    }
    // ─── ADMIN: Assign security to hostel or mess ─────────────
    /** Assign a SECURITY user to a hostel. */
    async assignSecurityToHostel(securityUserId, hostelId) {
        const user = await prisma.user.findUnique({ where: { id: securityUserId }, select: { role: true } });
        if (!user)
            throw ApiError.notFound("User not found");
        if (user.role !== "SECURITY")
            throw ApiError.badRequest("User is not a security personnel");
    ...
    async assignSecurityToMess(securityUserId, messId) {
        const user = await prisma.user.findUnique({ where: { id: securityUserId }, select: { role: true } });
        if (!user)
            throw ApiError.notFound("User not found");
        if (user.role !== "SECURITY")
        ...
        if (!mess)
            throw ApiError.notFound("Mess not found");
        securityHostelCache.delete(securityUserId);
        const updated = await prisma.user.update({
            where: { id: securityUserId },
            data: {
    ...
    async unassignSecurity(securityUserId) {
        securityHostelCache.delete(securityUserId);
     ...
     */
    async getStudentHistory(userId, year, month) {
        ...
        if (!student)
            return { hostel: null, days: [], summary: { total: 0, present: 0, onLeave: 0, absent: 0 } };
        const allocation = student.roomAllocations[0];
        if (!allocation)
            return { hostel: null, days: [], summary: { total: 0, present: 0, onLeave: 0, absent: 0 } };
