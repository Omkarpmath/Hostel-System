...
export class AttendanceController {
    ...
    async scanStudent(req, res, next) {
        try {
            ...
            if (!qrToken)
                return ApiResponse.error(res, 400, "QR token is required");
    ...
    async getRegister(req, res, next) {
        try {
            ...
            if (!hostelId)
                return ApiResponse.error(res, 400, "hostelId is required");
    ...
    async exportRegisterCSV(req, res, next) {
        try {
            const { hostelId, date } = req.query;
            if (!hostelId || !date)
                return ApiResponse.error(res, 400, "hostelId and date are required");
    ...
    // ─── ADMIN endpoints ─────────────────────────────────────
    async assignSecurity(req, res, next) {
        try {
            const { securityUserId, hostelId } = req.body;
            if (!securityUserId || !hostelId)
                return ApiResponse.error(res, 400, "securityUserId and hostelId are required");
    ...
    async unassignSecurity(req, res, next) {
        try {
            ...
            if (!securityUserId)
                return ApiResponse.error(res, 400, "securityUserId is required");
    ...
    async listSecurityUsers(_req, res, next) {
        try {
