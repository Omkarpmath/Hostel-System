...
export class AnnouncementService {
     ...
     */
    async calculateTargetCount(announcement) {
        const where = {};
     ...
     */
    async listAnnouncements(userId, role, filters) {
     ...
     */
    async getMyAnnouncements(userId, filters) {
        await this.reconcileLifecycle();
     ...
     */
    async getAnnouncementById(id, userId, role) {
        await this.reconcileLifecycle();
     ...
     */
    async createAnnouncement(userId, role, data) {
        const now = new Date();
     ...
     */
    async updateAnnouncement(id, userId, role, data) {
     ...
     */
    async deleteAnnouncement(id, userId, role) {
        const existing = await prisma.announcement.findUnique({ where: { id } });
        if (!existing) {
            throw ApiError.notFound("Announcement not found");
        }
    ...
    async markAllAsRead(userId) {
        ...
        const operations = activeAnnouncements.map((a) => prisma.announcementRead.upsert({
            where: {
                announcementId_userId: {
                    announcementId: a.id,
                    userId,
