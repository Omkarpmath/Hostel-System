...
export class OperationsService {
    ...
    async allocate(studentId, roomId, requestedBed) {
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (!hostel || hostel.isActive === false)
                throw ApiError.badRequest("Hostel is inactive or not found");
    ...
    async vacate(allocationId, actorId, actorRole = "ADMIN") {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const newStatus = room.status === "BLOCKED"
                ? "BLOCKED"
    ...
    async academicRollover(actorId, actorRole = "ADMIN") {
    ...
    async exportAttendanceShortageCsv() {
