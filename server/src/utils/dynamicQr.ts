...
export interface DynamicQrPayload {
  ...
  n: string;         // Random nonce for uniqueness
}
 ...
 */
export function verifyQrToken(rawToken: string): VerificationResult {
  ...
  if (token.startsWith("DQR_")) {
    ...
    try {
      ...
      const payload: DynamicQrPayload = JSON.parse(payloadJson);
