...
export class ReceiptService {
   ...
   */
  async processReceiptAndEmail(feeId: string, razorpayPaymentId: string, options?: { force?: boolean }) {
    try {
      ...
      try {
        if (razorpayPaymentId) {
          const payment: any = await razorpayClient().payments.fetch(razorpayPaymentId);
...
Bull Temple Road, Bengaluru - 560019`;
   ...
   */
  async getReceiptPdfByFeeId(feeId: string, requesterUserId: string, requesterRole: string): Promise<{ buffer: Buffer; filename: string }> {
    const fee = await prisma.fee.findUnique({
      where: { id: feeId },
      include: {
        student: {
